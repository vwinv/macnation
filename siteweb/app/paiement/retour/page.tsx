...
export default async function PaiementRetourPage({
  ...
  const rdvOk = paid && (Boolean(booking) || invoice?.kind === "rdv");
    ...
    <main className="flex min-h-dvh items-center justify-center bg-white px-5 py-12 text-center">
      <div className="w-full max-w-md rounded-2xl bg-gray-950 p-8 stroke-gradient [--stroke-opacity:0.2]">
        ...
        <h1 className="font-bebas mt-3 text-5xl text-black">
          {rdvOk ? "Rendez-vous pris" : paid ? "Paiement reçu" : "Paiement en cours"}
        </h1>
        <p className="mt-3 text-sm text-gray-400">
          {rdvOk
            ? `Ton rendez-vous${booking ? ` du ${booking.dateLabel} à ${booking.time}` : ""} est bien confirmé. On t’attend au salon.`
            : paid
              ? `${invoice?.number || "Commande"} · ${formatFcfa(invoice?.amount || 0)}. Merci, à tout à l’heure au salon.`
              : "Si tu as validé sur ton téléphone, le rendez-vous s’enregistre dès confirmation. Tu peux fermer cette page."}
        </p>
        <Link href={rdvOk ? "/compte" : "/"} className="btn-gold mt-8 inline-flex h-12 items-center justify-center rounded-lg px-6 text-sm font-medium">
          {rdvOk ? "Voir mon compte" : "Retour au site"}
        </Link>
      </div>
    </main>
  );
